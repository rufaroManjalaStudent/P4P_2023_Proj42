...
eye(size(bpq, 1))];
    V0(bpq) = T*vt0; %NOTE!! Raf made this elementwise 
...
% Calculate voltage, current and power based on the estimate 
    ...
    v0 = V0.*exp(li*A0);   i0 = Y*v0; pq0 = v0.*conj(i0);
...
Jp = [imag(K)-diag(imag(pq0)) ... 
	(real(K)+diag(real(pq0)))./(ones(nbs,1)*VO')];

Jq = [diag(real(pq0))-real(K) ... 
	(imag(K)+diag(imag(pq0)))./(ones(nbs,1)*V0')];

J = [Jp;Jq]; 
J(sort([bsl; nbs+bs1;nbs+bpv]),:)=[]; 
...
bus_sln(sort([bpv;bs1]),4:5) = [real(pq0(sort([bpv;bs1]))) 	imag(pq0(sort([bpv;bs1])))]; 
